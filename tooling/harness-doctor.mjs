...
if (ROLE === 'unknown') {
  blockers.push('không xác định được VAI của repo này: không có `.claude/harness-manifest.json` (dấu của repo đã áp) '
    + 'và cũng không có `cli.mjs` trong `tooling` (dấu của repo template — thứ DUY NHẤT không bao giờ ship xuống repo con). Harness tới đây bằng đường không ai theo dõi được — '
